...
test("offsetToColRow()", () => {
	...
	expect(offsetToColRow("test single line case that wraps", 25, colSize)).toEqual({
		row: 0,
		col: 25
	});
	expect(offsetToColRow("test single line case that wraps", 26, colSize)).toEqual({
		row: 1,
		col: 0
	});
	...
	expect(offsetToColRow("test multiple lines that wraps and\nalso\nnew\nlines", 25, colSize)).toEqual({
		row: 0,
		col: 25
	});
	expect(offsetToColRow("test multiple lines that wraps and\nalso\nnew\nlines", 26, colSize)).toEqual({
		row: 1,
		col: 0
	});
	expect(offsetToColRow("test multiple lines that wraps and\nalso\nnew\nlines", 35, colSize)).toEqual({
		row: 2,
		col: 0
	});
});
...
test("collectAutocompleteCandidates()", () => {
	...
	expect(collectAutocompleteCandidates(cbList, "")).toEqual(["a", "ab", "abc", "c", "cd", "cde"]);
	expect(collectAutocompleteCandidates(cbList, "a")).toEqual(["a", "ab", "abc"]);
	expect(collectAutocompleteCandidates(cbList, "ab")).toEqual(["ab", "abc"]);

	expect(collectAutocompleteCandidates(cbList, "ab ")).toEqual(["a", "ab", "abc", "b", "bc", "bcd", "c", "cd", "cde"]);
	expect(collectAutocompleteCandidates(cbList, "ab b")).toEqual(["b", "bc", "bcd"]);
});
