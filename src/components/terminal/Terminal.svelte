...
TOOLS_DEFAULT.forEach((d, i) => {
	...
	if (i > 0) d.reinit = true;
	// Special cases
	if (d.tool === "bowtie2") AUTOCOMPLETE_DEFAULT[d.tool] = [];
	else AUTOCOMPLETE_DEFAULT[d.program || d.tool] = [];
});
...
onMount(async () => {
	...
	const toolsEagerLoad = TOOLS_DEFAULT.filter((tool) => tools.includes(tool.tool) || tools.includes(tool.program)).map((tool) => {
		tool.loading = "eager";
		return tool;
	});
...
async function input(toPrint) {
	...
	if (toPrint) $xterm.write(toPrint);
	$xterm.focus();

	// Prepare prompt, e.g. "guest@sandbox$ "
	const prompt = $env["PS1"].replaceAll("\\u", $env["USER"]).replaceAll("\\h", $config.hostname);
	$xtermAddons.echo.read(`\u001b[1;34m${prompt}\u001b[0m`).then(exec).catch(console.error);
}

// Execute command
async function exec(cmd) {
	// Handle special `clear` command
	if (cmd == "clear") return input(ANSI_CLEAR);
...
function handleShortcuts(key) {
	...
	if (key.domEvent.ctrlKey && key.domEvent.key == "a") $xtermAddons.echo.setCursor(0);

	// Ctrl + E = End of line
	if (key.domEvent.ctrlKey && key.domEvent.key == "e") $xtermAddons.echo.setCursor(Infinity);

	// Ctrl + W = Delete last word
	if (key.domEvent.ctrlKey && key.domEvent.key == "w") $xtermAddons.echo.handleData("\x1b\x7F");
}
...
async function handleAutocomplete(data) {
	...
	if (args.length == 0) cacheAutocomplete = Object.keys(AUTOCOMPLETE);
	// Autocomplete subcommands
	else if (prgm in AUTOCOMPLETE) {
		// e.g. "samtools " --> "view   sort   index  idxstats"
		// e.g. "samtools i" --> "index   idxstats"
		if (args.length < 2) cacheAutocomplete = AUTOCOMPLETE[prgm];
	...
	if (cacheAutocomplete.length == 1 && cacheAutocomplete[0].endsWith("/")) appendExtraSpace = false;

	// Process autocomplete
	// If nothing to autocomplete, just add a space
	if (cacheAutocomplete.length == 0) {
		if (!input.endsWith(" ")) $xtermAddons.echo.handleCursorInsert(" ");
