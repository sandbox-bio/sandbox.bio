...
async function initTutorialFiles({ files, pwd }) {
	...
	if (files?.length > 0) {
		...
		for (let file of files) {
			...
			filenameParts.pop();
			if (filenameParts.length > 0) await exec(`mkdir -p ${pathDest}/${filenameParts.join("/")}`);
...
async function transform(cmd) {
	...
	// Handle aliases
	if (tool == "bowtie2") cmd.command.value = "bowtie2-align-s";
	else if (tool == "awk") cmd.command.value = "gawk";
	else if (tool == "ll") {
...
async function exec(cmd, callback = console.warn) {
	...
	// Input validation
	// -------------------------------------------------------------------------
	if (cmd.type == "subshell") throw "Error: subshells are not supported.";
	if (cmd.body) throw `Error: ${cmd.type} is not supported.`;
	...
	if (typeof cmd === "string") {
		...
		if (cmd.includes("awk") && awkVarMatch) {
			for (let match of awkVarMatch) cmd = cmd.replace(match[0], match[0].replace("=", ESCAPED_EQUAL_SIGN));
		...
		try {
			...
			if (error.name == "SyntaxError") throw "Unrecognized command";
			throw error;
		}
	}
	...
	if (Array.isArray(cmd)) {
		...
		if (
			...
			firstRedirects?.length === 1 &&
			firstRedirects[0]?.type === "redirectFd" &&
			firstRedirects[0]?.fd === 1 &&
			firstRedirects[0]?.op === ">" &&
			["view", "sort", "cat", "collate"].includes(firstArgs?.[0]?.value)
		) {
	...
	if (cmd.type == "command") {
		...
		if (cmd.command == "[[") throw "Error: Test expressions not supported";
		try {
			let output;
			...
			for (let redirect of cmd.redirects || []) {
				...
				if (redirect.type == "pipe") {
					...
					// Write to file
					if (redirect.op == ">") await utils.writeFile(path, "" + output); // convert to string if it's not already
			...
			// -----------------------------------------------------------------
			if (cmd.next && cmd.control != "||") output += await exec(cmd.next, callback); // FIXME: remove breakline and trim
...
const coreutils = {
	...
	history: (args) => {
		...
		if (args.d) {
			...
			if (index < 0 || index >= historyController.entries.length) return "Invalid history line number.";
	...
	rm: async (args) => {
		...
		const hasFlagR = args.r != null,
			hasFlagF = args.f != null;
		...
		for (const path of args._) {
			try {
				...
				// Delete files first
				for (let file of files) await _fs.unlink(file);
				// Then delete folders
				for (let folder of folders.reverse()) await _fs.rmdir(folder);
				// Finally, delete the remaining folder itself
				if (hasFlagR) await _fs.rmdir(path);
			} catch (error) {
	...
	mkdir: async (args) => {
		...
		if (hasFlagP) args._.unshift(args.p);
		...
		for (const path of args._) {
			...
			for (const subpath of subpaths.filter((s) => s !== "")) {
				try {
					...
					try {
						if (await _fs.stat(subpath)) continue;
					} catch (error) {}
					// Create folder
	...
	cp: async (args) => {
		...
		try {
			...
			// Copy data over
			if (args._[1] == ".") args._[1] = args._[0].split("/").pop();
	...
	curl: async (args) => {
		...
		// Output to file
		if (args.o || args.O) {
			if (args.O) args.o = url.split("/").pop();
...
const utils = {
	...
	getValue: async (arg) => {
		// Literal; support ~
		if (arg.type == "literal") {
			if (arg.value === "~" || arg.value?.startsWith("~/")) return arg.value.replaceAll("~", $env.HOME);
			return arg.value;
			// Variable
		} else if (arg.type == "variable") return $env[arg.name] || "";
		// Variable concatenation, e.g. echo "something $abc $def"
		else if (arg.type == "concatenation") return (await Promise.all(arg.pieces.map(utils.getValue))).join("");
		// Command Substitution, e.g. someprgm $(grep "bla" test.txt | wc -l)
		else if (arg.type == "commandSubstitution") return await exec(arg.commands);
		// Process substitution, e.g. bedtools -a <(grep "Enhancer" data.bed)
			...
			if (arg.value.endsWith("/")) arg.value = arg.value.slice(0, -1);
			...
			if (pathPattern === "*") return files.map((f) => `${pathBase}${f.name}`);
			...
			if (filesMatching.length > 0) return filesMatching;
			if (pathPattern == "") return [pathBase || "."];

			// If find no matches, return original glob value
			return arg.value;
		} else throw `Error: ${arg.type} arguments not supported.`;
	},
	...
	writeFile: async (path, contents, opts = { encoding: "utf8" }) => {
		...
		if (typeof contents === "string") contents = contents.replace(ansiRegex(), "");
...
const fsSave = async function () {
	...
	for (let path of filesToCache) {
		...
		try {
			...
			if (size > MAX_FILE_SIZE_TO_CACHE) continue;
		} catch (error) {
			console.warn(error);
			continue;
		}
		...
		if (path.endsWith("/")) folders[path] = true;
		else files[path] = await _fs.readFile(path, { encoding: "binary" });
	}
...
const fsLoad = async function () {
	...
	for (let path in files) await utils.writeFile(path, files[path], { encoding: "binary" });
...
async function fsTraverse(path) {
	...
	for (let file of files) {
		...
		paths.push(filePath);
		if (file.name.endsWith("/")) paths = paths.concat(await fsTraverse(filePath));
	}
	return paths;
}
